...
class KDTree
{
  ...
  private:
    ...
    KDTreeNode *root;
    size_t size;
    void copy(KDTreeNode *&subrootOurs,KDTreeNode *other);
    vector<Point<Dim>> kdpoints;
    void insert(KDTreeNode *&subRoot, int start, int end);
    ...
    void DestoryRecurs(KDTreeNode *&subNode);
    void fNNHelper(const Point<Dim>& query,int startL,int endR,
                                int dimen, double& dist,bool & first,Point<Dim>&currentBest) const;
    void buildTheTree(/*KDTreeNode *subRoot,*/int startL,int endR,int dim);
    void quickSelectALgo(int startL,int endR,int dimen,int getMid);

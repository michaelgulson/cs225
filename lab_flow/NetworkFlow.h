...
private:
  void insertingFlowEdges(Edge theEdge);
  void insertingResidualEdges(Edge theEdge,int whichWay);
  int FindMinPath(const std::vector<Vertex> & path,int findMinWeight) const;
  void settingTheFlow(Vertex one, Vertex two, int pathCapaci);
  void settingTheResidual(Vertex one, Vertex two, int pathCapaci);
  Graph & g_;
  Graph residual_;
  Graph flow_;
